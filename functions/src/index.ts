...
export const subscribeTokenToTopic = functions.https.onCall(
  async (request: functions.https.CallableRequest<SubscribeData>) => {
...
export const notifyPostOwnerOnReaction = https.onCall(async (request) => {
  ...
  const { postId, type, ownerId, postTitle } = request.data;
  if (!postId || !type || !ownerId || !postTitle) {
    throw new https.HttpsError("invalid-argument", "Faltan datos en la solicitud.");
  }
  ...
  const body = `Tu post "${postTitle}" recibió un ${type}.`;
...
const BAD_WORDS = ["sopa", "baka", "puerco", "alcornoque", "insecto"];
...
export const moderatePostContent = onDocumentWritten("posts/{postId}", async (event) => {
  ...
  BAD_WORDS.forEach(word => {
    ...
    updatedContent = updatedContent.replace(regex, "[!bip#]");
  });
